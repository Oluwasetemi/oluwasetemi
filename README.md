# Hello there; welcome 👋🏾

[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://engineering.altschoolafrica.com/)](https://engineering.altschoolafrica.com/) [![Website Badge](https://img.shields.io/badge/-oluwasetemi.dev-000000?style=for-the-badge&logo=Google-Chrome&logoColor=white&link=https://oluwasetemi.dev)](https://oluwasetemi.dev) [![Linkedin Badge](https://img.shields.io/badge/-setemiojo-blue?style=for-the-badge&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/setemiojo)](https://www.linkedin.com/in/setemiojo) [![Twitter Badge](https://img.shields.io/badge/-@setemiojo-1ca0f1?style=for-the-badge&logo=twitter&logoColor=white&link=https://twitter.com/setemiojo)](https://twitter.com/setemiojo)

A Full-stack JavaScript/TypeScript(Node and React) engineer with a sound technical background, with 7 years of experience working at small to mid-sized startups. My passion for building software products drives me, and I have valuable leadership experience leading engineering teams. Have worked with cutting-edge web technologies, such as React and Vue, to build software products that are tested using Vitest and Playwright in a Test-Driven Development approach, utilizing Agile methodologies. Have consistently worked with TypeScript, Nodejs, React, and Nextjs in a production environment. I write Python conveniently.

# 5 Recent Blog Posts from my [blog](https://oluwasetemi.dev)
<!-- blog start -->
[Introducing the new Set methods in ES2025](https://oluwasetemi.dev/blog/introducing-the-new-set-method-in-es2025/) - 10 months ago

[Designing Data-Intensive Applications - What I Learned from Part 1: Foundations of Data Systems](https://oluwasetemi.dev/blog/what-i-learned-from-part-1-foundations-of-data-systems/) - 10 months ago

[From 10 Lines to 100 Retries: Building a Git Clone Function That Doesn't Quit](https://oluwasetemi.dev/blog/from-10-lines-to-100-retries-building-a-git-clone-function-that-doesnt-quit/) - 11 months ago

[building-apollo-server-hono-integration](https://oluwasetemi.dev/blog/building-apollo-server-hono-integration/) - 11 months ago

[complete-guide-to-html-attributes](https://oluwasetemi.dev/blog/complete-guide-to-html-attributes/) - last year

[Introductory Guide to ES2021](https://oluwasetemi.dev/blog/introductory-guide-to-es-2021/) - 2 years ago
<!-- blog end -->

**Here's a quick summary about me**:

- 😊 Pronouns: He/him
- 💡 Fun fact: I'm currently a contract tutor at AltSchool Africa [School of Software Engineering](https://engineering.altschoolafrica.com/).
- 👨🏻‍💻 Streaming about JavaScript on Fridays and Reactjs on [youtube](https://youtube.com/@setemiojo), [twitch](https://twitch.tv/setemiojo) and [linkedin](https://www.linkedin.com/in/setemiojo/)
- 🖋️ I write about my experience as an Engineer on [Oluwasetemi](https://oluwasetemi.dev)
- 🌱 I’m well-experienced with TypeScript, JavaScript, ReactJS, and Python.
- 😊 I’m looking for opportunities to help contribute to open source projects.
- 💼 Job interests: Software Engineer, Frontend Engineer, or Full Stack Engineer (Senior level).
- 📫 You can also contact me by emailing setemiojo@gmail.com.



---

| ![Metrics](./metrics.svg) | <img align="center" src="https://github-readme-stats.vercel.app/api?username=oluwasetemi&show_icons=true&include_all_commits=true&hide_border=true" alt="Setemi's GitHub stats" /> | <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=oluwasetemi&langs_count=8&layout=compact&hide_border=true" alt="Setemi's GitHub stats" /> |
| ------------- | ------------- | ------------- |
